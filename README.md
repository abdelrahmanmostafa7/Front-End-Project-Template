...
5. **`src/lib/client.ts`**  
   - Axios instance: `NEXT_PUBLIC_BACKEND_URL`, JSON headers, **`X-TENANT-ID: dms`**, 60s timeout.  
...
8. **`next.config.ts`**  
   ...
   - **`images.remotePatterns`**: example S3 host (`dms-tenant.s3.us-west-2.amazonaws.com`) — adjust for your CDN.
...
- **Rename the package** in `package.json` (`name` is currently `dms-new-frontend-nextjs`).
